...
Tinytest.add('get user first name and last initial', function (test) {
  ...
  test.equal(msUser.getName(users[0]), 'Kayla');
});
...
Tinytest.add('convert UTC date time to user timezone', function (test) {
  ...
  test.equal(msUser.toUserTime(user, dtUTC, null), '2015-07-01 12:45:00+03:30');
});
...
//     }
//   };
//   test.equal(msUser.toUserTime(user, dtUTC, null), '2015-07-01 02:15:00-07:00');

//   // Should allow no user (in case not logged in) and still use a default.
//   test.equal(msUser.toUserTime(null, dtUTC, null), '2015-07-01 02:15:00-07:00');
// });
...
Tinytest.add('convert UTC date time to user timezone with custom format', function (test) {
  ...
  test.equal(msUser.toUserTime(user, dtUTC, null, 'MMM DD, YYYY @ h:mma'), 'Jul 01, 2015 @ 2:15am');
  test.equal(msUser.toUserTime(user, dtUTC, null, 'MM/DD/YY'), '07/01/15');
  test.isNotUndefined(msUser.toUserTime(user, dtUTC, null, 'fromNow'));
});