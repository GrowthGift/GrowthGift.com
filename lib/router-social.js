...
Router.route('metatags', {
  ...
  action: function() {
    ...
    if(socialScraper) {
      ...
      // game page
      if(reqUrl.slice(0,2) ==='g/') {
        var gameSlug =reqUrl.slice(2, reqUrl.length);
        var game = gameSlug ? GamesCollection.findOne({ slug: gameSlug },
         { fields: { title:1, image:1, gameRuleId:1 } }) : null;
        var gameRule = game ? GameRulesCollection.findOne({ _id: game.gameRuleId },
         { fields: { challenges:1 } }) : null;
        if( game && gameRule ) {
          meta.title =game.title;
          var image =ggGame.getImage(game)
          meta.image = ( image.indexOf('://') > -1 ) ? image : ( url1 + image );
          console.log(gameRule);
          meta.desc =gameRule.challenges[0].description;
        }
      }
